...
# Usage:
#   ./scripts/migrate.sh           # migrate using current venv / python on PATH
#   PYTHON=python3 ./scripts/migrate.sh
#   VENV=.venv ./scripts/migrate.sh  # optional: activate a venv before running
#
# Notes:
# - Expects to be run from the repository root.
# - Uses SQLite by default (see backend/pcwl_backend/settings.py).
# - If running first time, run: ./scripts/setup.sh
#
set -euo pipefail
...
if [[ "$PY_MAJOR" -ge 3 && "$PY_MINOR" -ge 13 ]]; then
  ...
  echo "       Then re-run: ./scripts/migrate.sh" >&2
  exit 1
fi
...
if ! "$PYTHON_BIN" -c "import django" >/dev/null 2>&1; then
  ...
  echo "[PCWL] Hint: run ./scripts/setup.sh to create a virtualenv and install requirements, then retry." >&2
  echo "[PCWL]       If you see 'permission denied', run: bash scripts/setup.sh" >&2
  echo "[PCWL]       Or make scripts executable once: chmod +x scripts/*.sh" >&2
  exit 1
fi
